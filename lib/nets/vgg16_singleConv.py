...
#-----------keras function definition---------------------------
  ...
  def single_relation_conv(self,input_map,MLP_units,channel): #fall feature map: slow version
    ...
    features = []
    for k1 in range(w):
      features1 = self.slice_1(pool5)
      pool5 = self.slice_2(pool5) # like cut layer one by one
      for k2 in range(h):
    ...
    mid_relations=slim.conv2d(relations_map, MLP_unit, [2*c,1], padding="VALID",trainable=is_training, weights_initializer=initializer, scope="mid_relations1")
    print("mid_relations:",mid_relations.shape)
    mid_relations=slim.conv2d(mid_relations, MLP_unit, [1,1], padding="VALID",trainable=is_training, weights_initializer=initializer, scope="mid_relations2")
    print("mid_relations2:",mid_relations.shape)
    mid_relations=slim.conv2d(mid_relations, channel, [1,1], padding="VALID",trainable=is_training, weights_initializer=initializer, scope="mid_relations3")
...
#----------------------------------------------------------
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      rn_map=single_relation_conv(pool5,512,512)
      '''
      w, h = pool5_shapes[1], pool5_shapes[2]
      print("pool_shape:",w,h)
      print("begin get features...")
      features = []
      for k1 in range(w):
        features1 = self.slice_1(pool5)
      ...
      print(relations_map.shape)
      print("beging get  mid relationships")
      mid_relations=slim.conv2d(relations_map, 512, [1024,1], padding="VALID",trainable=is_training, weights_initializer=initializer, scope="mid_relations1")
      print("mid_relations:",mid_relations.shape)
      ...
      print("mid_relations3:",mid_relations.shape)
      rn_map = slim.avg_pool2d(mid_relations,[1,81]) 
      print("rn:",rn_map.shape)
      '''
